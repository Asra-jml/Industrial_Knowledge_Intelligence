...
> **Purpose:** a single, always-current record of _what we built, how it works, and why we
> made each decision_ — for hackathon judges, interviewers, and teammates.
...
_(entries appended as tasks complete)_
...
_(full construction-rules table added when builder.py lands)_
...
2. **Ingest** (30 s): `python -m backend.ingestion.run_ingest` → incremental run finishes
   in ~12 s: _"475 documents, 8,050 chunks, 664 nodes, 589 edges + Neo4j upsert"_.
